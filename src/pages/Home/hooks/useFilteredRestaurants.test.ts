...
describe('useFilteredRestaurants', () => {
  ...
  it('filters restaurants by search query (name)', () => {
    ...
    expect(result.current.every(restaurant => 
  ...
  it('filters restaurants by search query (city)', () => {
    ...
    const { result } = renderHook(() =>
      useFilteredRestaurants(mockRestaurants, cityName, {})
    );

    expect(result.current.length).toBeGreaterThan(0);
    expect(result.current.every(restaurant => 
      restaurant.address.city.toLowerCase().includes(cityName)
    )).toBe(true);
  });
  ...
  it('filters restaurants by search query (cuisine)', () => {
    ...
    if (restaurantWithCuisine) {
      ...
      const { result } = renderHook(() =>
        useFilteredRestaurants(mockRestaurants, cuisineName, {})
      );

      expect(result.current.length).toBeGreaterThan(0);
      expect(result.current.some(restaurant => 
        restaurant.cuisines?.some(cuisine => 
          cuisine.name.toLowerCase().includes(cuisineName)
        )
      )).toBe(true);
    }
  });
  ...
  it('filters restaurants by new status', () => {
    const { result } = renderHook(() =>
      useFilteredRestaurants(mockRestaurants, '', { new: true })
    );
  ...
  it('filters restaurants by 4+ star rating', () => {
    ...
    expect(result.current.every(restaurant => 
      (restaurant.rating?.starRating ?? 0) >= 4
    )).toBe(true);
  });
  ...
  it('applies multiple filters correctly', () => {
    ...
    expect(result.current.every(restaurant => 
      restaurant.deliveryCost === 0 && restaurant.isOpenNowForDelivery
    )).toBe(true);
  });
  ...
  it('applies search query and filters together', () => {
    ...
    result.current.forEach(restaurant => {
      ...
      expect(
        ...
        restaurant.cuisines?.some(cuisine => 
          cuisine.name.toLowerCase().includes(searchQuery)
        )
      ).toBe(true);
    });
  });
  ...
  it('handles undefined restaurants array', () => {
    const { result } = renderHook(() =>
      useFilteredRestaurants(undefined, 'test', {})
    );
